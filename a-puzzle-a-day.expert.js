...
document.querySelector('#dayofweek').value = dayofweek;
...
const pieceColors = [
  'purple', 'green', 'yellow', 'red', 'blue', 'pink', 'orange', 'gray', 'brown', 'cyan'
];
...
function init() {
  ...
  const targets = [
    ...
    document.querySelector('#dayofweek').value
  ];
...
function forwardCut(selections, nextIndex) {
  ...
  [0, 1, 2, 3].forEach(y => checkedBoard[7][y] = 1);
  let x = y = 0;
  const sections = [];
  ...
  while (checkedBoard.some(r => r.some(i => i === 0))) {
    if (checkedBoard[x][y] === 0) {
      ...
      while (point = stack.pop()) {
        console.log(point);
        // left
        if (point[0] < 7 && point[1] !== 0 || point[0] === 7 && point[1] !== 4) {
        ...
        // down
        if (point[0] !== 7 && (point[0] !== 6 || (point[1] >= 4 && point[1] <= 6))) {
          if (checkedBoard[point[0] + 1][point[1]] === 0) {
            checkedBoard[point[0] + 1][point[1]] = 1;
            stack.push([point[0] + 1, point[1]]);
            section.push([point[0] + 1, point[1]]);
...
function _moveRight(coordinates) {
  return coordinates.map(coordinate => ([coordinate[0], coordinate[1] + 1]));
}

function moveRight(coordinates) {
  return validatePosition(_moveRight(coordinates));
}

function _moveDown(coordinates) {
  return coordinates.map(coordinate => ([coordinate[0] + 1, coordinate[1]]));
}

function moveDown(coordinates) {
  let current = _moveDown(coordinates);
  let validated = validatePosition(current);
  if (validated !== null) return validated;
  for (var i = 0; i < 6; i++) {
    current = _moveRight(current);
    let validated = validatePosition(current);
    if (validated !== null) return validated;
  }
  return null;
...
function validatePosition(coordinates) {
  for (const coordinate of coordinates) {
    if (
      ...
      (coordinate[0] == 7 && (coordinate[1] < 4 || coordinate[1] > 6)) ||
      (coordinate[0] < 7 && coordinate[1] >= boardNames[coordinate[0]].length)
    ) {
      return null;
    }
  }
